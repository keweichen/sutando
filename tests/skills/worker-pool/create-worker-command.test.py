...
Run: python3 tests/skills/worker-pool/create-worker-command.test.py
"""
...
class TestTheRosterCannotGoStale(Base):
    ...
    def test_the_resolved_runtime_reaches_the_roster_and_the_advertisement(self):
        # Any runtime the resolver supports; the assertion is about the passthrough.
        self.assertEqual(self.run_cli("--label", "Research", "--runtime", "claude"), 0)
        wid = self.spawned[0]
        self.assertEqual(pr.load_roster(self.ws)["workers"][wid]["runtime"], "claude")
        got = json.loads((self.ws / "state" / "pool-advertisement.json").read_text())
        self.assertEqual(got["profile_workers"][wid], {"label": "Research", "runtime": "claude"})
